...
function setupStaticRecipeToggle(){
  const toggle = document.getElementById('toggleCard');
  if(!toggle || document.getElementById('recipeDetail')) return;
  toggle.addEventListener('click', () => {
    document.body.classList.toggle('recipe-card-mode');
    toggle.textContent = document.body.classList.contains('recipe-card-mode') ? 'Show field guide version' : 'Quick recipe version';
  });
}

document.addEventListener('DOMContentLoaded',()=>{ setupNav(); setupStaticRecipeToggle(); buildCalculator(); buildRatioTable(); buildRecipes(); buildTroubleshooting(); });
